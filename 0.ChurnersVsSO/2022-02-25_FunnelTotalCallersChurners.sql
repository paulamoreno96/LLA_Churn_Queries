...
WITH
  ...
  WHERE
    ...
    AND SUBAREA <> "0 A 30 DIAS" AND SUBAREA <> "30 A 60 DIAS" AND SUBAREA <> "60 A 90 DIAS" AND SUBAREA <> "90 A 120 DIAS" AND SUBAREA <> "120 A 150 DIAS"
    AND SUBAREA <> "150 A 180 DIAS" AND SUBAREA <> "MAS DE 180" AND MOTIVO <> "LLAMADA  CONSULTA DESINSTALACION"
...
/*Consulta final que cruza los contratos con tiquetes con los churners que se desinstalan hasta 2 meses después de tener un tiquete*/
SELECT
  EXTRACT (MONTH FROM l.FECHA_APERTURA) AS MES, COUNT(DISTINCT c.CONTRATOSO)
FROM CONTRATOSLLAMADAS l INNER JOIN CHURNERSMES c ON l.CONTRATO = c.CONTRATOSO
