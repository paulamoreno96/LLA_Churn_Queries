...
ACTIVOSPLANBB AS(
 SELECT DISTINCT EXTRACT(MONTH FROM t.FECHA_EXTRACCION) AS MES, t.ACT_ACCT_CD,
    ...
    WHEN PD_BB_PROD_NM IN(
        ...
        CASE WHEN Max(C_CUST_AGE) <= 3 THEN "<3M"
        WHEN Max(C_CUST_AGE) > 3 AND Max(C_CUST_AGE) <= 6 THEN "03-6M"
        WHEN Max(C_CUST_AGE) > 6 AND Max(C_CUST_AGE) <= 9 THEN "06-9M"
        WHEN Max(C_CUST_AGE) >9 AND Max(C_CUST_AGE) <= 12 THEN "09-1A"
        WHEN Max(C_CUST_AGE) >12 AND Max(C_CUST_AGE) <= 24 THEN "1-2 A"
        WHEN Max(C_CUST_AGE) >24 AND Max(C_CUST_AGE) <= 36 THEN "2-3 A"
        WHEN Max(C_CUST_AGE) > 26 AND Max(C_CUST_AGE) <=48 THEN "3-4 A"
...
GROUP BY t.ACT_ACCT_CD, MES, VELOCIDADBB)

SELECT t.MES, t.VELOCIDADBB, t.TENURE, COUNT(DISTINCT t.ACT_ACCT_CD)
FROM ACTIVOSPLANBB t 
