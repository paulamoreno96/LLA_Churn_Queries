WITH CHURNPLANBB AS(
 SELECT DISTINCT ACT_ACCT_CD,MAX(CST_CHRN_DT) AS Maxfecha,
    ...
    WHEN PD_BB_PROD_NM IN(
        'I11B_INTERNET FTTH CT 8M/5M GRUPO 2',
...
GROUP BY ACT_ACCT_CD, VELOCIDADBB
 HAVING EXTRACT (MONTH FROM MAX(CST_CHRN_DT)) = EXTRACT (MONTH FROM MAX(FECHA_EXTRACCION))),
CRUCECHURN AS
(
 SELECT DISTINCT c.ACT_ACCT_CD, c.Maxfecha AS FECHA,c.VELOCIDADBB, min(t.ACT_ACCT_INST_DT) as MinInst
 FROM CHURNPLANBB c INNER JOIN `gcp-bia-tmps-vtr-dev-01.gcp_temp_cr_dev_01.2022-02-02_CRM_BULK_FILE_FINAL_HISTORIC_DATA_2021_D` t
 ON c.ACT_ACCT_CD = t.ACT_ACCT_CD AND c.Maxfecha = t.CST_CHRN_DT
 GROUP BY c.ACT_ACCT_CD, c.Maxfecha, c.VELOCIDADBB
),
TENUREACTIVOS AS(
...
SELECT EXTRACT (MONTH FROM FECHA)AS MES, VELOCIDADBB, TENURE, COUNT (DISTINCT ACT_ACCT_CD) as NumChurners
FROM TENUREACTIVOS
