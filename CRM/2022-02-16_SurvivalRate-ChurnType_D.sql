...
CRUCECOLLECTIONSCRM AS(
    ...
    FROM CHURNERSCRM LEFT JOIN MOROSOSCRM ON CONTRATOCRM = CONTRATO  AND DATE_DIFF (Maxfecha, FECHA_FACTURA, DAY) >= 60 --OR DATE_DIFF (MaxfechaCRM, FECHA_FACTURA, DAY) >=90)
...
CLASIFICACIONCHURN AS(
    ...
    GROUP BY CONTRATOCRM, MAXFECHA, TipoChurn
),

/*Subconsulta que define los meses de churn*/
MESESCHURN AS(
SELECT DISTINCT CONTRATOCRM,
...
SELECT 
Mesesitos, COUNT(DISTINCT a.CONTRATOALTA)
