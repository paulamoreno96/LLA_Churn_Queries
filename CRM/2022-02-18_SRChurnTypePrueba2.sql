...
/*CLASIFICACIONCHURN AS(
    SELECT DISTINCT Mesesitos, CHURNERCRM,
    CASE WHEN INVOLCHURN IS NULL THEN CHURNERCRM END AS Voluntario,
    CASE WHEN INVOLCHURN IS NOT NULL THEN CHURNERCRM END AS Involuntario,
    FROM MESESCHURN
    GROUP BY CHURNERCRM, MaxFecha, Mesesitos, Voluntario, Involuntario
)*/

/*Consulta final que extrae los churners de cada mes en base al mes de alta definido*/
SELECT 
Mesesitos, COUNT(DISTINCT a.CONTRATOALTA), COUNT(DISTINCT VOLUNTARIO), COUNT (DISTINCT INVOLUNTARIO)
