...
CHURNERSCRM AS(
  SELECT DISTINCT RIGHT(CONCAT('0000000000',ACT_ACCT_CD) ,10) AS CONTRATOCRM, MAX(DATE(CST_CHRN_DT)) AS Maxfecha
    FROM `gcp-bia-tmps-vtr-dev-01.gcp_temp_cr_dev_01.2022-02-16_FINAL_HISTORIC_CRM_FILE_2021_D`
...
CRUCECOLLECTIONSCRM AS(
    SELECT DISTINCT CONTRATOCRM, CONTRATO, Maxfecha
    FROM CHURNERSCRM LEFT JOIN MOROSOSCRM ON CONTRATOCRM = CONTRATO  
    AND ((DATE_DIFF (Maxfecha, FECHA_FACTURA, DAY) >= 60 AND TIPO_SERVICIO IN ('INTERNET', 'FTTH (JASEC)', 'HOGARES CONECTADOS')
    OR DATE_DIFF (Maxfecha, FECHA_FACTURA, DAY) >=90) AND TIPO_SERVICIO IN ('HOGARES CONECTADOS','TELEFONIA','CABLE TICA'))
    GROUP BY CONTRATOCRM, CONTRATO,Maxfecha
),
CLASIFICACIONCHURN AS(
    SELECT DISTINCT CONTRATOCRM, Maxfecha,
    ...
    GROUP BY CONTRATOCRM, MAXFECHA, VOLUNTARIO, INVOLUNTARIO
),

/*Subconsulta que define los meses de churn*/
MESESCHURN AS(
SELECT DISTINCT CONTRATOCRM,VOLUNTARIO, INVOLUNTARIO,
...
FROM AMBASALTAS a INNER JOIN MESESCHURN c ON c.CONTRATOCRM =a.CONTRATOALTA
