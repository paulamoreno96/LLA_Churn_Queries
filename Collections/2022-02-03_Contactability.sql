...
'Promesa de Pago'
    ...
    THEN 'CONTACTO_EFECTIVO' ELSE 'NO_EFECTIVO' END AS DETALLE
...
SELECT MG.MES_ASIGNACION 
    , MG.BUCKET_MORA 
    , COUNT(DISTINCT CASE WHEN GI.DETALLE = 'CONTACTO_EFECTIVO' THEN MG.CONTRATO_ID END) AS CONTRATOS_CONTACTADOS
    , COUNT(DISTINCT MG.CONTRATO_ID) AS CONTRATOS_TOTALES
    , (ROUND(
        COUNT(DISTINCT CASE WHEN GI.DETALLE = 'CONTACTO_EFECTIVO' THEN MG.CONTRATO_ID END) 
