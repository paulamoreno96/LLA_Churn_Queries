...
SELECT C.Contrato AS CONTRATO_ID
    ...
    , CASE 
      ...
      DATE_DIFF(c.FECHA_PAGO, C.FECHA_FACTURA, DAY) <= 90 AS PAGO_ANTES
...
SELECT MG.MES_ASIGNACION 
 --, MG.BUCKET_MORA 
    , COUNT(DISTINCT CASE WHEN NOT(GI.Promesa = "SI") AND MG.PAGO_ANTES THEN MG.CONTRATO_ID END) AS CONTRATOS_PAGO
    , COUNT(DISTINCT CASE WHEN NOT(GI.Promesa = "SI") THEN MG.CONTRATO_ID END) AS CONTRATOS_SIN_PROMESA
    , ROUND(
        COUNT(DISTINCT CASE WHEN NOT(GI.Promesa = "SI") AND MG.PAGO_ANTES THEN MG.CONTRATO_ID END) 
        / NULLIF(
            COUNT(DISTINCT CASE WHEN NOT(GI.Promesa = "SI") THEN MG.CONTRATO_ID END)
            , 0)
        , 4) AS RATIO
