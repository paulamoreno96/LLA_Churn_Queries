...
PRODUCTOS_MAXIMA_FECHA AS(
    SELECT DISTINCT MF.ACT_ACCT_CD, PD_BB_PROD_NM, PD_TV_PROD_CD, PD_VO_PROD_NM, MF.FECHA_BASE
    FROM  `gcp-bia-tmps-vtr-dev-01.gcp_temp_cr_dev_01.2022-02-16_FINAL_HISTORIC_CRM_FILE_2021_D` CRM
    INNER JOIN MAXIMA_FECHA_MES MF ON CRM.ACT_ACCT_CD = MF.ACT_ACCT_CD AND MF.FECHA_BASE = CRM.FECHA_EXTRACCION
    WHERE DATE_TRUNC (FECHA_EXTRACCION, MONTH) = '2021-07-01'
)
, LAST_VALS AS (
...
FROM `gcp-bia-tmps-vtr-dev-01.gcp_temp_cr_dev_01.2022-02-16_FINAL_HISTORIC_CRM_FILE_2021_D` CRM
    ...
    INNER JOIN MAXIMA_FECHA_MES MF ON MF.FECHA_BASE = CRM.FECHA_EXTRACCION AND MF.ACT_ACCT_CD = CRM.ACT_ACCT_CD
)
, LAST_MRC AS (
SELECT MONTH
    , MONTH_ORDER_ROW
...
FROM LAST_VALS LV INNER JOIN PRODUCTOS_MAXIMA_FECHA PM ON LV.CST_CUST_ID = PM.ACT_ACCT_CD
)
, PIVOT_VALS AS (
