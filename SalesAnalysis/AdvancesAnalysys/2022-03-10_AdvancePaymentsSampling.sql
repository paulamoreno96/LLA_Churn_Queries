...
ALTASINTERNET30 AS (
SELECT RIGHT(CONCAT('0000000000',Contrato) ,10) AS CONTRATOALTA, Formato_Fecha,Categor__a_Canal, Activo as ActivoBB
FROM `gcp-bia-tmps-vtr-dev-01.gcp_temp_cr_dev_01.2022-01-20_CR_ALTAS_V3_2021-01_A_2021-12_T`  
WHERE Tipo_Venta="Nueva"
AND Categor__a_Canal = "Agentes Autorizados"
AND ACTIVO = "I0141_INTERNET CT 100MBPS/5MBPS V3"
...
GROUP BY Contrato, Formato_Fecha, Categor__a_Canal, Activo
),
ALTASNEXTGENTV AS(
SELECT RIGHT(CONCAT('0000000000',Contrato) ,10) AS CONTRATOALTA, Formato_Fecha,Categor__a_Canal, Activo as ActivoTV
FROM `gcp-bia-tmps-vtr-dev-01.gcp_temp_cr_dev_01.2022-01-20_CR_ALTAS_V3_2021-01_A_2021-12_T`  
WHERE Tipo_Venta="Nueva"
AND Categor__a_Canal = "Agentes Autorizados"
AND ACTIVO = "NextGen TV"
AND (Tipo_Cliente = "PROGRAMA HOGARES CONECTADOS" OR Tipo_Cliente="RESIDENCIAL" OR Tipo_Cliente="EMPLEADO")
AND extract(year from Formato_Fecha) = 2021 
AND Subcanal__Venta<>"OUTBOUND PYMES" AND Subcanal__Venta<>"INBOUND PYMES" AND Subcanal__Venta<>"HOTELERO" AND Subcanal__Venta<>"PYMES – NETCOM" 
AND Tipo_Movimiento= "Altas por venta"
...
FROM `gcp-bia-tmps-vtr-dev-01.gcp_temp_cr_dev_01.2022-01-20_CR_ALTAS_V3_2021-01_A_2021-12_T`  
WHERE Tipo_Venta="Nueva"
AND Categor__a_Canal = "Oficina"
AND ACTIVO = "TELEFONIA PLAN FULL RESIDENCIAL"
AND (Tipo_Cliente = "PROGRAMA HOGARES CONECTADOS" OR Tipo_Cliente="RESIDENCIAL" OR Tipo_Cliente="EMPLEADO")
AND extract(year from Formato_Fecha) = 2021 
AND Subcanal__Venta<>"OUTBOUND PYMES" AND Subcanal__Venta<>"INBOUND PYMES" AND Subcanal__Venta<>"HOTELERO" AND Subcanal__Venta<>"PYMES – NETCOM" 
AND Tipo_Movimiento= "Altas por venta"
...
ALTASPLAN2P AS(
    SELECT i.CONTRATOALTA, i.Formato_Fecha, i.ActivoBB, t.ActivoTV
    FROM ALTASINTERNET30  i LEFT JOIN ALTASNEXTGENTV t ON i.CONTRATOALTA = t.CONTRATOALTA and i.Formato_Fecha = t.Formato_Fecha
   WHERE t.CONTRATOALTA IS NULL
    --INNER JOIN ALTASVOIP v on i.CONTRATOALTA = v.CONTRATOALTA and i.Formato_Fecha = v.Formato_Fecha
),
...
FROM ALTASCRM y INNER JOIN ALTASPLAN2P x ON y.CONTRATOALTACRM=x.CONTRATOALTA
